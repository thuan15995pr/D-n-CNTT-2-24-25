...
def re_syn(true_data, predicted_data):
    # Select the last data point of each window
    ...
    # Create a new row with only 0
    new_row = pd.DataFrame({'True_Data_origin': [None], 'Predicted_Data_origin': [None]})  # Other columns can be filled with None or appropriate default values

    # Merge new row with original data
...
def integrate_Predicted_data(symbol, sentiment_type, n):
    ...
    for model_dir in model_dirs:
        # Find folders containing specified symbols in each model's directory
        for folder in os.listdir(model_dir):
            if prediction_name in folder or captal_sentiment in folder or captal_nonsentiment in folder:
                ...
                # Find CSV files containing specified symbols in the found folder
    ...
    # Merge all model's predicted data into a DataFrame
    merged_df = pd.concat([df.set_index(df.index) for df in dataframes], axis=1)
    print(merged_df)
    # Delete duplicate True_data columns, keep the first one
...
if __name__ == "__main__":
    # Specify symbols and model directories
